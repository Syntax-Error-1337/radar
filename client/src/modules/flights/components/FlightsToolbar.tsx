...
export const FlightsToolbar: React.FC<FlightsToolbarProps> = ({
    ...
    return (
        <div className="absolute top-0 left-0 right-0 p-2 bg-intel-panel/50 border-b border-intel-panel backdrop-blur-md z-10 flex items-center gap-4 pointer-events-auto flex-wrap">
            ...
            {/* ── Altitude slider ───────────────────────────── */}
            <div className="flex items-center gap-2">
                <label className="text-[10px] font-bold tracking-widest text-intel-text-light whitespace-nowrap">MAX ALT (m)</label>
                <input
                    type="range"
            ...
            {/* ── Speed slider ──────────────────────────────── */}
            <div className="flex items-center gap-2">
                <label className="text-[10px] font-bold tracking-widest text-intel-text-light whitespace-nowrap">MAX SPD (m/s)</label>
                <input
                    type="range"
                    className="accent-intel-accent w-20"
                    min="0" max="1000" step="50"
                    value={filters.speedMax}
                    onChange={e => setFilter('speedMax', Number(e.target.value))}
                />
